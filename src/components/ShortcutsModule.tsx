...
import { Terminal, Copy, Check, Search, ExternalLink, ArrowRight, Sparkles } from 'lucide-react';
...
export const ShortcutsModule: React.FC<ShortcutsModuleProps> = ({
  ...
  // Safety check to prevent React from crashing if data is missing
  const safeCommands = Array.isArray(SCRIPT_COMMAND_DEFINITIONS) ? SCRIPT_COMMAND_DEFINITIONS : [];

  // Ultra-safe string builder to prevent VS Code parsing crashes
  const commandListString = safeCommands
    .map((cmd) => "- [" + (cmd?.command || "") + "]: " + (cmd?.description || ""))
    .join("\n");

  const masterExternalPrompt = [
    "You are an Expert Educational Scriptwriter and YouTube Educator. Generate a teaching script for the ARPITON presentation engine.",
    "",
    "=== SCRIPT FORMAT RULES ===",
  ...
  const filtered = safeCommands.filter((item) => {
    if (!item) return false;
    const matchesCat = selectedCategory === 'All' || item.category === selectedCategory;
    const sTerm = search.toLowerCase();
    const matchesSearch =
      (item.command || '').toLowerCase().includes(sTerm) ||
      (item.description || '').toLowerCase().includes(sTerm) ||
      (item.example || '').toLowerCase().includes(sTerm);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-800 gap-3">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] px-2 py-0.5 rounded font-mono bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 font-bold">MODULE 7</span>
            <h2 className="text-xl font-bold text-white">SCRIPT COMMAND ENGINE & EXTERNAL AI PROMPT</h2>
          </div>
          <p className="text-xs text-slate-400 mt-1">Browse visual commands or copy the Master System Prompt to ChatGPT.</p>
        </div>
        <div className="flex items-center gap-2 text-xs font-mono text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 rounded-lg">
          ...
          <span>{safeCommands.length} Commands</span>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Copy this prompt into external AI to generate formatted scripts.</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <a href="https://chatgpt.com" target="_blank" rel="noreferrer" className="flex items-center gap-1 px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-slate-200 border border-slate-700 transition">
              <span>ChatGPT</span> <ExternalLink className="w-3.5 h-3.5"/>
            </a>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 space-y-4">
            ...
            <div className="flex gap-1 overflow-x-auto pb-1">
              {categories.map((cat) => (
                <button
                  ...
                  className={`text-xs px-2.5 py-1.5 rounded-lg whitespace-nowrap transition ${selectedCategory === cat ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-800'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[520px] overflow-y-auto pr-1">
            {filtered.map((item) => (
              <div key={item.command} className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 transition flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-mono font-bold text-indigo-300 px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/20">[{item.command}]</span>
                    <span className="text-[10px] text-slate-500">{item.category}</span>
                  </div>
                  <p className="text-xs text-slate-300 mb-2 leading-relaxed">{item.description}</p>
                  <div className="p-2 rounded bg-slate-950 border border-slate-800/80 text-[11px] font-mono text-slate-400 truncate">{item.example}</div>
                </div>
                <div className="mt-3 pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2">
                  <button onClick={() => handleCopyCmd(item.example, item.command)} className="flex items-center gap-1 text-[11px] text-slate-400 hover:text-white transition">
                    {copiedCmd === item.command ? <><Check className="w-3.5 h-3.5 text-emerald-400"/><span className="text-emerald-400">Copied!</span></> : <><Copy className="w-3.5 h-3.5"/><span>Copy Syntax</span></>}
                  </button>
                  <button onClick={() => handleInsertIntoPlayground(item.example)} className="text-[11px] px-2 py-1 rounded bg-indigo-600/20 text-indigo-300 font-medium transition">Insert +</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="rounded-xl bg-slate-900/80 border border-slate-800 p-5 flex flex-col h-full">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-3">
              <div>
                <h3 className="text-xs font-bold text-white uppercase tracking-wide">Script Staging Area</h3>
              </div>
              <button
              ...
              >
                <span>Open in Slides</span> <ArrowRight className="w-3.5 h-3.5"/>
              </button>
            </div>
            <textarea
              ...
              rows={18}
              className="flex-1 w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs font-mono text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-y"
            />
            <div className="mt-3 flex justify-end">
              <button onClick={() => setPlaygroundText('')} className="text-slate-500 hover:text-slate-300 text-xs">Clear</button>
            </div>
          </div>
        </div>
      </div>
